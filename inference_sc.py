...
def test(
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("StoryCrafter")
...
def test2(
    ...
    for id in ref_image:
        if id == '':
            ref_image = torch.zeros(3, 512, 512).float()
    ...
    for i, image in enumerate(output):
        images.append(image[0])
        images[i].save(os.path.join(logdir, f"output_{i}.png"))
    return images
...
if __name__ == "__main__":
    ...
    num_sample_per_prompt = 15
    mixed_precision = "fp16"
    stage = 'multi-image-condition' 
    
    prompt = "A boy named Tom found a stary cat and hoking it in his arms."
    prev_p = ["In this cartoon, the left is a sitting boy and the right is a stray cat."]
    ref_image = ["./boy_cat.png"]
