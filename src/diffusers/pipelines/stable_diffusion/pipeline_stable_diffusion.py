...
def rescale_noise_cfg(noise_cfg, noise_pred_text, guidance_rescale=0.0):
    """
    Rescale缩放 `noise_cfg` according to `guidance_rescale`. Based on findings of [Common Diffusion Noise Schedules and
    ...
    guidance_rescale: cfg权重参数
    """
    # 计算noise_pred_text 除第0维外所有维度的std
    std_text = noise_pred_text.std(dim=list(range(1, noise_pred_text.ndim)), keepdim=True)
    # 计算noise_cfg 除第0维外所有维度的std
    std_cfg = noise_cfg.std(dim=list(range(1, noise_cfg.ndim)), keepdim=True)
    # rescale the results from guidance (fixes overexposure)调整noise_cfg，权重为std_text / std_cfg 
...
class StableDiffusionPipeline(
    ...
    #推断 准备潜变量，用于StableDiffusion模型的推断过程
