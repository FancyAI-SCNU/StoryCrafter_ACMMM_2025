...
class StableDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.register_modules( #将模型中的不同组件注册到当前的 pipeline 实例中 后面可以用self调用
            vae=vae,
        ...
        self.vae_scale_factor = 2 ** (len(self.vae.config.block_out_channels) - 1)  # 8 #self.vae...包含了 VAE 网络每个块的输出通道数（也就是每个层的输出特征数量） 计算缩放因子
    ...
    def __call__(
        ...
        for p_prompt in prev_prompt: #prev prompt=3
        ...
        ref_image_num = t_image_prompts.shape[0] # 3
        
        zero_image_prompt = t_image_prompts[0] * 0  # 像素全为0
        zero_image_prompt = self.vae.encode(zero_image_prompt).latent_dist.sample()  # zero image latent sample
        zero_image_prompt = zero_image_prompt * 0.18215 # [B,4,64,64]
        zero_image_prompt = zero_image_prompt.repeat(num_images_per_prompt, 1, 1, 1)  # 第一个维度（batch）将 zero_image_prompt 重复 num_images_per_prompt 次，但在其他维度上保持不变。 输出[B * num_images_per_prompt, 4, 64, 64]
        zero_image_prompts = []
        for i in range(ref_image_num):  # ref_image_num=3
            zero_image_prompts.append(zero_image_prompt)  # [3 x (B,4,64,64)] 每个参考图像生成一个零图像潜在特征
        ...
        for t_image_prompt in t_image_prompts:
            ...
            image_prompts.append(new_image_prompt)   # [3 x (B,4,64,64)]
