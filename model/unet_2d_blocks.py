...
def get_down_block(
    ...
    down_block_type = down_block_type[7:] if down_block_type.startswith("UNetRes") else down_block_type #如果 down_block_type 以 "UNetRes" 开头，则将其去掉，只保留后面的部分。
    if down_block_type == "DownBlock2D":
        ...
        return CrossAttnDownBlock2D( #和unet_2d_condition down_block = get_down_block里的一样
            num_layers=num_layers,#2
            in_channels=in_channels,#320 640
            out_channels=out_channels,#320 1280
            temb_channels=temb_channels,#1280
            add_downsample=add_downsample,#t
            resnet_eps=resnet_eps,#
            resnet_act_fn=resnet_act_fn,
            resnet_groups=resnet_groups,
            downsample_padding=downsample_padding,
...
class CrossAttnDownBlock2D(nn.Module):
    def __init__( #值会通过super().__init__()从上面的函数继承
        self,
        in_channels: int,
        out_channels: int,
        temb_channels: int,
        ...
        for i in range(num_layers):
            in_channels = in_channels if i == 0 else out_channels #如果是第一个层，保持输入通道为 in_channels，否则设置为 out_channels，确保每层的输入通道正确。
            resnets.append( #创建一个 ResnetBlock2D 实例并添加到 resnets 列表中。这个块会使用给定的参数（如通道数、时间嵌入、归一化等）进行初始化。
                ResnetBlock2D(
                    in_channels=in_channels,
                    out_channels=out_channels,
                    temb_channels=temb_channels,
            ...
            attentions.append(
                Transformer2DModel( #创建一个 Transformer2DModel 实例并添加到 attentions 列表中。该模型的参数也由传入的参数决定。
                    attn_num_head_channels,
...
class CrossAttnUpBlock2D(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            res_skip_channels = in_channels if (i == num_layers - 1) else out_channels #循环一次这里改 在循环一次下一行改 这个一次是指循环2次？
